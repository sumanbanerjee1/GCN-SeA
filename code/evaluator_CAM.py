...
from utils_my.dbPointer import queryResultVenues
from utils_my.delexicalize import *
...
def parseGoal(goal, d, domain):
    ...
    if 'info' in d['goal'][domain].keys():
        if domain == 'train':
            # we consider dialogues only where train had to be booked!
            if 'book' in d['goal'][domain].keys():
                goal[domain]['requestable'].append('reference')
            if 'reqt' in d['goal'][domain].keys():
...
def parse_goal(data,filename):
    goal = {'restaurant':{}}
    g = data['goal']
    inf={}
    for i in g['constraints']:
        inf[i[0]] = i[1]
    goal['restaurant']['booking']={}
    goal['restaurant']['informable']=inf
    goal['restaurant']['requestable']=g['request-slots']
    
    return goal,{'restaurant':g['request-slots']}

def evaluateModel(dialogues, val_dials, mode='test'):
    """Gathers statistics for the whole sets."""
    
    delex_dialogues = val_dials
    ...
    for filename, dial in dialogues.items():
        ...
        goal,requestables = parse_goal(data, filename)
    ...
    if mode == 'valid':
        ...
        print('Test Corpus Matches : %2.2f%%' % (matches / float(total) * 100))
        print('Test Corpus Success : %2.2f%%' % (successes / float(total) * 100))
        ...
        for domain in goal.keys():
            ...
            if '<v.name>' in sent_t:
                if domain in ['restaurant']:
                    ...
                    venues = queryResultVenues(domain, realDialogue['meta'][t*2 + 1])
#                    print(venues)
...
def evaluateRealDialogue(dialog, filename):
    ...
    for domain in domains_in_goal:
        ...
        if 'info' in dialog['goal'][domain].keys():
            if 'name' in dialog['goal'][domain]['info'].keys():
                venue_offered[domain] = '[' + domain + '_name]'
